...
<head>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function priceUpdate() {
   	...
   	// 데이터 불러오기
   	formattedAdultPrice = ${requestScope.rd.adultprice};
    formattedChildPrice = ${requestScope.rd.childprice};
   	document.querySelector('#price').textContent = "성인👩 " +  formattedAdultPrice.toLocaleString() + "원 │ 아동👶 " + formattedChildPrice.toLocaleString() + "원";
	...
	const getStatus = "${requestScope.rd.status}";
...
function check() {
  ...
  let ans = confirm("수정하시겠습니까?");
	  if (ans == true) {
		  fm.action="${pageContext.request.contextPath}/reservation/${requestScope.rd.ridx}/reservationModifyAction.do";
		  fm.method="post";
		  fm.submit();
	}
  ...
  <%@ include file="/WEB-INF/header.jsp" %>
    ...
    <form class="detail pb-5" name="frm">
      <input type="hidden" class="form-control" name="cidx" value="${requestScope.rd.getCidx()}">
      <input type="hidden" class="form-control" id="startday" name="startday" value="${requestScope.rd.getStartday()}">
      <div class="card text-center mb-3">
        <h3 class="card-title fw-bold mb-4">${requestScope.rd.getTitle()}</h3>
        <p class="card-text text-body-secondary pt-4 border-top-dashed">${requestScope.rd.getSummary()}</p>
      </div>
        ...
        <div class="row">
          ...
          <div class="col-6">            
            <div class="row g-0 border rounded shadow-sm p-4">
              ...
              <div class="pl-4">
                <p id="fromTo">${requestScope.rd.startday} ~ ${requestScope.rd.endday}</p>
              </div>
              ...
              <div class="row mb-3">
                <div class="col-6 pl-4">
                  ...
                  <input type="text" class="form-control" id="reservationname" required="" value="${requestScope.rd.name}" name="name" placeholder="홍길동">
                </div>
                <div class="col-6 pr-4">
                  <label for="phone" class="form-label">연락처</label>
                  <input type="text" class="form-control" id="reservationphone" required="" value="${requestScope.rd.phone}" name="phone" placeholder="01012345678">
                </div>
              </div>
              
              <p class="fw-bold mb-1 pt-3 border-top-dashed">5. 상태</p>
                <div class="d-flex">
	            	<div class="form-check me-3">
		              <input type="radio" name="status" class="form-check-input status" id="waiting" value="대기">
		              <label class="form-check-label" for="waiting">대기</label>
		            </div>
		            <div class="form-check me-3">
		              <input type="radio" name="status" class="form-check-input status" id="confirmed" value="확정">
		              <label class="form-check-label" for="confirmed">확정</label>
		            </div>
		            <div class="form-check">
		              <input type="radio" name="status" class="form-check-input status" id="cancel" value="취소">
      ...
      <div class="text-center">
        <button class="btn btn-primary mb-3" type="button" onClick="check()">수정</button>
        <button class="btn btn-primary mb-3" type="button"  onclick="history.back();">뒤로</button>
      </div>
    </form>
